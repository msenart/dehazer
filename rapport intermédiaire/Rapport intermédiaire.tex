...
\\[1em]
Le principal désavantage de cette méthode est qu'elle est très lente à s'exécuter : elle devient impossible à appliquer sur des grandes images. On est on contraint d'appliquer l'algorithme sur des images de petite résolution (D'où l'apparition de petits carrés sur l'image). Cette contrainte nous amène donc à compresser les images et à en réduire leur qualité. Cela en devient problématique sur certaines images :
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.27\textwidth}
        \centering
        \includegraphics[width=\textwidth]{hazed_images/4.jpg}
        \caption{{\scriptsize image enbrumée quelconque compressée}}
        \label{fig:image1}
    \end{subfigure}
    % Deuxième image
    \begin{subfigure}[b]{0.27\textwidth}
    ...
    \label{fig:deux_images}
\end{figure}
En fait, beaucoup d'artefacts issus de la compression d'image altèrent le rendu final.
On a ainsi beaucoup de mal à déterminer la qualité du \textit{dehazer} en se contenant de petites images.
\\[1em]
\begin{large}
\textbf{Objectifs :}
\end{large}
